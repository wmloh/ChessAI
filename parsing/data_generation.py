...
from parsing.parsing_constant import WHITE_WIN, BLACK_WIN, STATE_DEPTH, ACTION_DEPTH
...
# SAMPLING_RATE = expon.pdf(range(1, SAMPLING_THRESHOLD + 1),
#                           scale=1 / np.log(AVERAGE_BRANCHING_FACTOR))
# SAMPLING_RATE = SAMPLING_RATE / SAMPLING_RATE.sum()
...
def sample_intermediate_states(game, states=None, labels=None, actions=None, sampling_rate=0.1):
    ...
    For a particular game, converts each board state into Tensors where the first
        n=len(sampling_rate) is randomly sampled depending on the probability and returns
        the states, labels and actions
    ...
    sample_array = np.random.binomial(1, p=sampling_rate, size=(100,))  # sampled probabilities
    ...
    outcome = 1 if outcome == WHITE_WIN else -1 if outcome == BLACK_WIN else 0
    ...
    for move in game.mainline_moves():
        ...
        # restarts cycle if exceeds
        if prob_count >= len(sample_array):
            prob_count = 0

        if not sample_array[prob_count]:  # skips if 0
            current_board.push(move)
            flip = not flip  # inverts flip
            outcome = outcome * -1  # alternating pattern of win/lose but constant if draw
            continue
